...
static const Preset kPresets[] = {
    ...
    { L"ズーム 拡大 (Ctrl+ホイール上)",     L"zoom_in" },
    { L"ズーム 縮小 (Ctrl+ホイール下)",     L"zoom_out" },

    { L"── マウスのボタン ──",              NULL },
    { L"中クリック",                        L"click:middle" },

    { L"── ブラウザ・タブ ──",              NULL },
    /* 上の 3 つはキー入力を通らないので、矢印キーや F5 を握っているページ
       (Alt+Left や F5 が効かないサイト)でも止められない。既定はこちら。
       どの appcmd を相手が受けるかは tools\appcmd_sweep.ps1 で実測した。
       ブラウザ 2 種で確実に効いたのは戻る・進む・更新の 3 つだけである。 */
    { L"戻る (ブラウザーバック)",           L"appcmd:back" },
    { L"進む (ブラウザーフォワード)",       L"appcmd:forward" },
    { L"更新 (ブラウザーリフレッシュ)",     L"appcmd:refresh" },
    { L"全画面 (F11)",                      L"f11" },
    ...
    /* ここから下は「普段は選ばないもの」。一覧は上から読むので、使う頻度の
       高いものを上に置き、用途が限られるものを下へ落としてある。

       ほかの常駐アプリのトリガーとして使うためのキー。同時押しを保っている
       あいだ押しっぱなしになるので、「押している間だけ効く」タイプにも使える。
       F13〜F15 は普通のキーボードに無いので、誤爆の心配がない。
       修飾キー単体(Ctrl / Shift)は [記録] で入れられるので、ここには出さない。 */
    { L"── 特殊キー ──",                    NULL },
    { L"F13",                               L"f13" },
    { L"F14",                               L"f14" },
    { L"F15",                               L"f15" },

    /* 上の「ブラウザ・タブ」にある戻る・進む・更新を、キーを押す形でやる版。
       ページに横取りされうるので普段は上を使う。ここに残してあるのは、
       ブラウザではなく「キーそのもの」を相手に届けたいときのため
       (ほかの常駐アプリのトリガーにするなど)。 */
    { L"── 同じことをキーでする ──",        NULL },
