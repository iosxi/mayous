...
指定方法は 2 通りあります。
  ...
  3. 押している間ずっと押しっぱなしにする
     一覧の「押している間ずっと」から選ぶか、hold:f13 のように直接入力すると、
     同時押しを保っているあいだ、そのキーを押しっぱなしにします。マウスの
     ボタンを離すとキーも離されます。拡大鏡のような「押している間だけ効く」
     アプリと組み合わせるための指定です。
...
・割り当てたキーは、既定で 120 ミリ秒押してから離しています。キーボードフックを
  使わず「キーが今押されているか」を一定間隔で見に行く作りのアプリ(ゲームや
  拡大鏡など)は、これより短い押下を丸ごと見落とすためです。取りこぼされる場合は
  mayous.ini の KeyHoldMs を伸ばしてください。逆に文字入力欄でキーが連打されて
  しまう場合は縮めてください。
  相手が「押している間だけ効く」タイプなら、前述の「押している間ずっと」を
  使うほうが確実です。
