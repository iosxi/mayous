...
左クリックは一切触りません。「先に押す側」にはできない仕様です(後述)。
...
先に押すボタンは 右・サイドボタン1・サイドボタン2 の 3 つ。
そこへ後から押すものとして 左・右・中・サイドボタン1・サイドボタン2・
ホイール上・ホイール下 の 7 つを組み合わせられます(計 18 通り)。

さらに「登録キー」として、キーボードのキーを後から押す側にできます。
たとえば「右クリックを押しながら F13」で「タブを閉じる(Ctrl+W)」のように使えます。

サイドボタンと中ボタンは、単独で押したときの動作も差し替えられます。
「サイドボタン1 で中クリック」「中ボタンでオートスクロール」のように使えます。

中ボタンは「先に押す側」にはできません(オートスクロールと衝突するため)。
「後から押す側」としては使えます。
...
指定方法は 2 通りあります。
  ...
  1. 一覧から選ぶ
     ウィンドウ操作・スクロール(水平ホイール・オートスクロール・
     Ctrl+ホイールのズーム)・ブラウザ/タブ・編集・メディアなど
     約 40 種類が用意してあります。
...
・モード中はカーソルがその場に止まり、マウスを動かすとその移動量が
  そのままスクロールになります(縦も横も)
・もう一度クリックするか Esc で抜けます
・入った場所に丸い目印が出ます
・速さは同じタブで調整できます(大きいほど速い。既定 100%)

この割り当てをすると、中ボタン本来のクリックは無くなります(押して離すのが
モードの入口になるため)。中クリックが必要なときは、サイドボタンの単独クリックに
「中クリック」を割り当ててください。
...
「停止する条件」タブで設定します。書き方は 1 行に 1 つ、2 種類だけです。
...
* の位置で一致のしかたが決まります(大文字小文字は区別しません)。
  ...
  title:Minecraft     ちょうどこれ         (完全一致)
  title:Minecraft*    これで始まるもの     (前方一致)
  title:*メモ帳*      どこかに含むもの     (部分一致)

ウィンドウ名でも止められるのは、java.exe のように 1 つの実行ファイルが
まったく別のアプリのウィンドウを何枚も出すことがあるからです。Minecraft と
そのランチャがまさにそれで、実行ファイル名で止めると両方止まってしまいます。

打ち込まなくても足せます。下の一覧に今動いているアプリが並ぶので、
選んで [▲ ファイル名追加] か [▲ ウィンドウ名追加] を押すだけです
(▲ は「上の欄へ入れる」の意味)。
ウィンドウ名で足すと末尾に * が付いた前方一致になります。

判定は 1 秒ごとに行うので、ウィンドウ名が変わればすぐ追随します。

なお、全機能の ON / OFF はトレイアイコンのメニューにあります。
...
キーボードのキーを自分で決められます。
  ...
  2. 組み合わせたいキーを 1 つ押します(例: F13)
  3. 右のドロップダウンで、そのときの動作を選びます(例: タブを閉じる (Ctrl+W))

これで「右クリックを押しながら F13」でタブが閉じるようになります。
登録キーはタブごとに別々なので、「右クリック + F13」と「サイドボタン1 + F13」に
違う動作を割り当てられます。

・マウスのボタンを押していないときは、そのキーは何も変わらず素通しします。
  普段使わないキー(F13〜F24 など)を選んでおくと安心です。
・登録を消したいときは、[記録] の画面で何も押さずに [OK] を押します。
...
・押したまま同じ組み合わせを繰り返した場合(ホイールを同じ方向へ回し続けるなど)は、
  いったん離してから少し間を空けて押し直します。離した瞬間が短すぎると、
  2 回目以降が無かったことになるためです。この間隔は設定画面の
  「同じキーの押し直し」から 120 / 80 / 40 / 20 ミリ秒の 4 段階で選べます(既定 40)。
  長いほど確実に届き、短いほど速く出ます。

  別のキーへ切り替わる場合は間を空けません。ホイール上下に別々のキーを
  割り当てておくと、どちらへ回しても即座に出ます。
...
・左クリックは「先に押す側」にはできません。そうすると押下が離すまで
  アプリに届かなくなり、ウィンドウの切り替えもドラッグの起点も兼ねる
  左クリックでは、遅れがそのまま「反応が鈍い」として体感に出るためです。
  以前は設定画面に「左クリック」タブがありましたが、動作の安定を優先して
  v13 でいったん取りやめました。左クリックは mayous をまったく通りません。
  「後から押す側」としてはこれまでどおり使えます。

・ゲームには使わないでください。アンチチートが低レベルフックを不正ツールと
  みなす可能性があります。既定で「フルスクリーンのアプリが前面のときは
  停止する」が ON になっていますが、ボーダーレスウィンドウのゲームなどは
  設定画面の「停止する条件」タブに足しておいてください。
...
・ほかのマウス常駐ソフトと併用しないでください。X-Mouse Button Control の
  ような同種のツールや、MouseGestureL.ahk、Logitech G HUB・Razer Synapse などの
  ボタン割り当てと同時に使うと挙動が不安定になります。特にサイドボタンは
  これらに奪われていることが多いので、mayous で使う場合は先方の割り当てを
  外してください。

  サイドボタンが思ったのと違う動きをするときは、まず mayous を終了させて
  確かめてください。終了させても同じなら、それは先方のツールの差し替えです。

  先方に任せたいボタンは、mayous 側では「先に押すボタン」そのものを使わない
  (そのタブの割り当てを全部「なし」にする)のが確実です。組み合わせだけを
  「なし」にした場合、mayous はいったん預かった押下を注入し直してから後の
  ボタンを通すので、ジェスチャー系のツールとは相性がよくありません。
